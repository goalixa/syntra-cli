#!/usr/bin/env python3
"""
Syntra REPL - Enhanced interactive mode.
...
def main():
    """Main entry point for syntra-repl command."""
    import argparse

    parser = argparse.ArgumentParser(
        description="Syntra REPL - Enhanced interactive mode",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  syntra-repl                    # Start with default server
  syntra-repl http://localhost:8000  # Start with custom server
        """
    )
    parser.add_argument(
        "server",
        ...
        help="Syntra API server URL (default: http://localhost:8000)"
    )

    args = parser.parse_args()
    repl = SyntraREPL(server=args.server)
    repl.run()
